...
envelope. Everything else in `MusterConfig` is read once — a source is opened when the
...
`MusterConfig` after this was written is reported as needing a restart until someone
teaches the reload to carry it. Wrong in the direction that tells the truth."""


def sections_needing_restart(before: MusterConfig, after: MusterConfig) -> tuple[str, ...]:
