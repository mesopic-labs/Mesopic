...
from muster.exporters.fanout import ExporterFanout, build_exporters
...
def test_only_enabled_exporters_are_built(
    ...
    monkeypatch.setenv("MUSTER_WEBHOOK_SECRET", "not-a-real-one")
...
def test_a_webhook_whose_secret_env_var_is_unset_is_refused(
    ...
    monkeypatch.delenv("MUSTER_WEBHOOK_SECRET", raising=False)
    config = MusterConfig.model_validate(raw_config)

    with pytest.raises(ConfigError, match="MUSTER_WEBHOOK_SECRET"):
