...
from muster.types import CameraId, EventKind
...
def test_a_fresh_handle_is_due_for_restart_immediately(config: MusterConfig) -> None:
    handle = WorkerHandle(FRONT_DOOR, config=config, entry=partial(emit_then_exit))

    assert handle.restart_due(now=0.0)


def test_backoff_grows_with_consecutive_failures(config: MusterConfig) -> None:
...
def test_backoff_is_capped(config: MusterConfig) -> None:
...
def test_a_successful_run_clears_the_backoff(config: MusterConfig) -> None:
