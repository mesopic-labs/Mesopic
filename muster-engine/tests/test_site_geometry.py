...
Fixtures build on `examples/muster.yaml` for the same reason `test_config_validation.py`
...
from muster.analytics import site_geometry
from muster.analytics.site_geometry import SiteGeometry
from muster.config.schema import MusterConfig
from muster.types import CameraId, LineId, MetricName, NormPoint, ZoneId, ZoneRole
...
def _compile(example: dict[str, Any], **sections: list[dict[str, Any]]) -> SiteGeometry:
    ...
    return SiteGeometry.compile(MusterConfig.model_validate({**example, **sections}))
...
def test_compile_groups_geometry_by_the_camera_it_belongs_to(example: dict[str, Any]) -> None:
    ...
    geometry = SiteGeometry.compile(MusterConfig.model_validate(example))
