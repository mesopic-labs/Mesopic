...
class FakeEngine:
    ...
    def __init__(self, path: Path, config: MusterConfig) -> None:
        self.path = path
        self.config = config
        self.saved: list[str] = []
    ...
    async def save_config(self, document: str) -> MusterConfig:
        ...
        self.config = MusterConfig.model_validate(yaml.safe_load(document))
        return self.config
...
@pytest.fixture
def config(config_file: Path) -> MusterConfig:
...
@pytest.fixture
def engine(config_file: Path, config: MusterConfig) -> FakeEngine:
...
def _app(config: MusterConfig, store: Store, engine: FakeEngine, **kw: Any) -> Any:
    return create_app(
        config=config,
        store=store,
...
@pytest.fixture
async def client(
    config: MusterConfig, store: Store, engine: FakeEngine
