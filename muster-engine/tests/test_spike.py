...
from muster.spike import SpikeStats, frame_line, run_spike
...
def test_frame_line_carries_ts_fps_and_track_foot_points() -> None:
    ...
    assert "\n" not in line, "one frame is one line, or `muster spike | jq` breaks"
...
def test_run_spike_emits_a_final_stats_line_when_the_stream_ends() -> None:
    ...
    `muster spike --stats` against a short clip prints frames and no numbers at all."""
...
def test_run_spike_closes_the_source_even_when_the_consumer_stops_early() -> None:
    """`muster spike | head -1` closes the generator; the socket must still be released."""
