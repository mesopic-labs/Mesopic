...
config = MusterConfig.model_validate(
...
def _aggregator(config: MusterConfig) -> Aggregator:
...
def _folded(config: MusterConfig, events: list[RawEvent]) -> list[MetricRow]:
...
def test_trim_leaves_metric_rows_alone(store: Store, config: MusterConfig) -> None:
...
def test_the_configured_window_is_applied_in_hours(store: Store, config: MusterConfig) -> None:
