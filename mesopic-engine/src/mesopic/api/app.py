...
| `/config`        | Render, validate, and hot-reload `mesopic.yaml`                    |
...
from mesopic.api.calibration import _same_origin, calibration_router
from mesopic.api.config_view import ConfigDocument, ConfigSaver, config_router
...
def create_app(
    *,
    ...
    def current() -> MesopicConfig:
        return live_config

    def _adopt(saved: MesopicConfig) -> None:
        nonlocal live_config
        live_config = saved

    app = FastAPI(title="Mesopic", docs_url=None, redoc_url=None, openapi_url=None)
