...
§13.1 makes `mesopic.yaml` authoritative — the store's tables are its compiled form and the
...
from mesopic.api.forms import bounded_body, declared_too_large, form_field
from mesopic.config.changes import sections_needing_restart
...
"""Read `mesopic.yaml`'s text. `None` for an engine built without a file behind it."""

ConfigSaver = Callable[[str], Awaitable[MesopicConfig]]
