...
from mesopic.api.auth import WriteGuard
from mesopic.api.calibrate import GeometryEdit, merge_geometry
from mesopic.api.rows import GeometryRow, count_for
from mesopic.config.schema import CameraConfig, FrigateSource, MesopicConfig
from mesopic.errors import ConfigError, SnapshotUnavailableError
from mesopic.types import CameraId

if TYPE_CHECKING:
    from fastapi.templating import Jinja2Templates

    from mesopic.api.app import GeometrySaver, Snapshotter
...
def calibration_router(
    *,
    current: Callable[[], MesopicConfig],
    adopt: Callable[[MesopicConfig], None],
    ...
    async def camera_snapshot(
        ...
        if isinstance(camera.source, FrigateSource):
            ...
            raise HTTPException(
                ...
                "frame to draw on — set its zones and lines in mesopic.yaml",
            )
        if snapshot is None:
