...
place to read to learn what a running box consists of. `mesopic run` is a thin caller.
...
from mesopic.config.schema import ApiConfig, LineConfig, MesopicConfig, ZoneConfig
from mesopic.config.writer import read_document, write_document, write_geometry
...
DATA_DIR_ENV_VAR = "MESOPIC_DATA_DIR"
...
def store_path(
    *,
    ...
    Deliberately not a config key. `mesopic.yaml` describes the *site* — cameras, geometry,
    thresholds — and where its own compiled form is written is a property of the
    deployment, which is exactly what the image already expresses with `MESOPIC_DATA_DIR`.
...
class Engine:
    ...
    async def save_config(self, document: str) -> MesopicConfig:
        ...
        the whole save took effect — see `mesopic.config.changes`.
        """
        if self.config_path is None:  # pragma: no cover - the app never offers the route
            msg = "this engine was built without a config file to write back to"
            raise ConfigError(msg)
