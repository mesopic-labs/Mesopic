...
from mesopic.aggregator.aggregator import EXIT_GRACE_S, Aggregator, bucket_of
...
from mesopic.supervisor.handle import WorkerEntry, WorkerHandle, WorkerReport
from mesopic.supervisor.worker import run_camera_worker
from mesopic.types import CameraId, EventKind, FrameTs, MinuteBucket, RawEvent
...
class Supervisor:
    ...
    async def snapshot(
        ...
        *,
        ...
        RTSP session is opened and `mesopic.api` never imports a codec. What comes back is
    ...
    async def reload(self, config: MesopicConfig) -> None:
