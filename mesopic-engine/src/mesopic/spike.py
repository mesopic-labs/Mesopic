...
* **Output is JSON lines on stdout, one object per line.** `mesopic spike | jq` and
  `mesopic spike > run.jsonl` both work, and P1.7's harness parses rather than scrapes.
...
from mesopic.types import FrameTs, Track
...
def run_spike(
    *,
    ...
    try:
        ...
        # Also runs on GeneratorExit — `mesopic spike | head -1` must still release the
