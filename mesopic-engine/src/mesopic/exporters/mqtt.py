...
from mesopic.types import CameraId, MetricName, MetricRow, SiteId
...
class MqttExporter:
    ...
    **Known limitation:** a renamed or deleted zone leaves its retained discovery config on
    ...
    def _topic(self, row: MetricRow) -> str:
        ...
        A trailing empty segment (`mesopic/front-door/occupancy/`) is a topic nobody can
    ...
    def _unique_id(self, row: MetricRow) -> str:
        parts = ["mesopic", self._site_id, row.camera_id, row.metric.value]
        if row.scope_id is not None:
            parts.append(row.scope_id)
        return "_".join(parts)
    ...
    def _config(self, row: MetricRow, unique_id: str) -> dict[str, object]:
        ...
        config: dict[str, object] = {
            ...
            "device": {
                "identifiers": [f"mesopic_{self._site_id}_{row.camera_id}"],
                "name": self._camera_names.get(row.camera_id, row.camera_id),
                "manufacturer": "Mesopic",
                "model": "Camera",
            },
        }
