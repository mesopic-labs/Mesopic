...
from mesopic.analytics.metrics.heatmap import pack_counts, unpack_counts
from mesopic.config.schema import MesopicConfig
from mesopic.errors import StoreError
from mesopic.store.store import BASELINE_SCHEMA_VERSION, Store
...
def test_applying_a_changed_config_updates_in_place(store: Store, config: MesopicConfig) -> None:
...
def test_dropping_a_camera_from_config_does_not_delete_its_history(
    ...
    Editing a camera out of `mesopic.yaml` is a routine thing to do — retiring a lens,
...
def test_a_fresh_database_is_stamped_at_the_baseline_version(tmp_path: Path) -> None:
    ...
    store = Store(tmp_path / "mesopic.db")
...
def test_a_migration_bumps_the_schema_version(tmp_path: Path) -> None:
    ...
    store = Store(tmp_path / "mesopic.db", migrations_dir=migrations)
...
def test_dropping_a_camera_from_config_does_not_delete_its_grids(
    ...
    thing standing between a tidied-up `mesopic.yaml` and months of lost history."""
    store.upsert_heatmaps([_grid(120.0)])
