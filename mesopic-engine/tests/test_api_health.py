...
from mesopic.api.health import (
    DISK_PRESSURE_PCT,
    CameraHealth,
    DiskHealth,
    HealthStatus,
...
def test_a_camera_the_supervisor_runs_reports_its_worker_state(config: MesopicConfig) -> None:
    states = {
...
def test_a_camera_disabled_in_config_is_reported_as_disabled(config: MesopicConfig) -> None:
    ...
    partly_disabled = MesopicConfig.model_validate(parsed)
    running = {
        camera.camera_id: WorkerReport(state=CameraState.STREAMING, consecutive_failures=0)
...
def test_every_configured_camera_appears_exactly_once(config: MesopicConfig) -> None:
...
def test_the_failure_count_is_reported_not_assumed(config: MesopicConfig) -> None:
...
def test_a_camera_reports_the_frame_time_and_fps_its_worker_sent(config: MesopicConfig) -> None:
...
def test_a_camera_whose_worker_has_not_reported_still_says_so(config: MesopicConfig) -> None:
...
def test_a_stalled_camera_degrades_the_engine(config: MesopicConfig) -> None:
...
def test_an_engine_whose_every_camera_has_stalled_is_down(config: MesopicConfig) -> None:
...
def test_down_is_still_decided_from_the_cameras_alone(config: MesopicConfig) -> None:
