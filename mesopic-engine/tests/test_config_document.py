...
from mesopic.config.changes import LIVE_SECTIONS, sections_needing_restart
from mesopic.config.schema import MesopicConfig, ZoneConfig
from mesopic.config.writer import read_document, write_document
from mesopic.errors import ConfigError
from mesopic.types import CameraId, MetricName, ZoneId, ZoneRole
...
CONFIG = MesopicConfig.model_validate(yaml.safe_load(DOCUMENT))
...
def test_only_the_three_sections_the_reload_carries_are_live() -> None:
    ...
    allow-list of nine: `sections_needing_restart` walks `MesopicConfig`'s fields and
    ...
    assert set(LIVE_SECTIONS) <= set(MesopicConfig.model_fields), "a typo here is a silent lie"
