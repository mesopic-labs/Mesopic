...
from mesopic.api.auth import SESSION_COOKIE, Credential, WriteGuard
...
async def _save_geometry(zones: Sequence[ZoneConfig], lines: Sequence[LineConfig]) -> MesopicConfig:
    del zones, lines
...
async def test_a_session_admits_a_write(
    ...
    async def save(zones: Sequence[ZoneConfig], lines: Sequence[LineConfig]) -> MesopicConfig:
...
def test_every_write_route_requires_a_session(config: MesopicConfig, store: Store) -> None:
...
def test_every_read_route_is_deliberately_open_or_deliberately_guarded(
    ...
    `/config` renders `mesopic.yaml`, which may hold an inline RTSP URL — the single value
