"""`mesopic run` — the composition root, and how it comes apart.

`make run` has shelled to `mesopic run --config ./mesopic.yaml` since P0; until P3.1 the
...
from mesopic.config.schema import ApiConfig, MesopicConfig, ZoneConfig
from mesopic.errors import ConfigError
from mesopic.runner import DATA_DIR_ENV_VAR, Engine, build_server, store_path
...
async def test_the_server_is_given_the_configured_bind(config: MesopicConfig, store: Store) -> None:
    engine = Engine(config, store, entry=run_until_stopped)
    server = SpyServer()
...
async def test_the_api_can_be_switched_off(config: MesopicConfig, store: Store) -> None:
    """An engine with no dashboard is a supported deployment, not a broken one."""
    headless = MesopicConfig.model_validate({**config.model_dump(), "api": {"enabled": False}})
...
async def test_metrics_is_served_from_the_fanouts_own_prometheus_exporter(
    ...
    """
    scraped = MesopicConfig.model_validate(
    ...
    assert "mesopic_metric" in exposition
...
async def test_a_server_that_exits_stops_the_workers(config: MesopicConfig, store: Store) -> None:
...
async def test_sigterm_shuts_the_engine_down(config: MesopicConfig, store: Store) -> None:
...
def test_the_store_lives_beside_the_config_by_default(tmp_path: Path) -> None:
    """A self-hoster who ran `mesopic run --config ./mesopic.yaml` gets `./mesopic.db`."""
    assert store_path(config_path=tmp_path / "mesopic.yaml", data_dir=None, env={}) == (
        tmp_path / "mesopic.db"
    )


def test_the_data_dir_env_var_wins_over_the_config_location(tmp_path: Path) -> None:
    """The image sets `MESOPIC_DATA_DIR=/data` and mounts one volume there."""
    volume = tmp_path / "data"

    resolved = store_path(
        config_path=tmp_path / "etc" / "mesopic.yaml",
        data_dir=None,
        env={DATA_DIR_ENV_VAR: str(volume)},
    )

    assert resolved == volume / "mesopic.db"
...
def test_an_explicit_data_dir_wins_over_the_env_var(tmp_path: Path) -> None:
    ...
    resolved = store_path(
        config_path=tmp_path / "mesopic.yaml",
        data_dir=explicit,
        env={DATA_DIR_ENV_VAR: str(tmp_path / "ignored")},
    )

    assert resolved == explicit / "mesopic.db"
...
def test_run_never_prints_a_source_url(tmp_path: Path, config: MesopicConfig) -> None:
...
@pytest.fixture
def config_file(tmp_path: Path, config: MesopicConfig) -> Path:
...
async def test_an_engine_with_no_config_file_cannot_save(
    store: Store, config: MesopicConfig, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_the_command_gives_the_engine_the_config_it_was_pointed_at(
    tmp_path: Path, config: MesopicConfig, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    class SpyEngine:
        def __init__(self, config: MesopicConfig, store: Store, **kwargs: Any) -> None:
            del config, store
            seen.update(kwargs)
...
PASSWORD_VAR = "MESOPIC_TEST_ADMIN_PASSWORD"  # noqa: S105 - the variable's name, not a password
PASSWORD = "correct-horse-battery-staple"  # noqa: S105 - a fixture, not a credential


def _with_credential(config: MesopicConfig, env_var: str | None) -> MesopicConfig:
...
async def test_without_a_password_env_the_engine_has_no_credential(
    store: Store, config: MesopicConfig
) -> None:
    """The default config, and the reason `mesopic run` still starts without a password."""
