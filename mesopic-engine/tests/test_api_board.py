...
from mesopic.api.health import CameraHealth
...
def test_a_configured_scope_with_no_data_shows_no_value(config: MesopicConfig) -> None:
...
def test_a_counting_tile_sums_the_window(config: MesopicConfig) -> None:
    rows = [
...
def test_a_stateful_tile_shows_the_newest_bucket(config: MesopicConfig) -> None:
...
def test_the_heatmap_gets_no_tile(config: MesopicConfig) -> None:
...
def test_a_tile_exists_for_every_configured_scope_and_metric(config: MesopicConfig) -> None:
    tiles = tiles_for(config, rows=[])
...
def test_a_configured_metric_with_no_data_still_gets_a_chart(config: MesopicConfig) -> None:
...
def test_a_chart_puts_every_scope_on_one_time_axis(config: MesopicConfig) -> None:
...
def test_a_scope_missing_a_bucket_gets_a_hole_not_a_shift(config: MesopicConfig) -> None:
...
def test_a_scope_keeps_one_colour_across_every_chart(config: MesopicConfig) -> None:
...
def test_the_slots_come_from_the_config_not_the_rows(config: MesopicConfig) -> None:
...
def test_a_window_with_no_rows_is_wholly_unexposed(config: MesopicConfig) -> None:
...
def test_a_bucket_marks_only_the_cell_it_falls_in(config: MesopicConfig) -> None:
...
def test_the_newest_bucket_lands_inside_the_strip(config: MesopicConfig) -> None:
...
def test_a_bucket_older_than_the_window_is_not_drawn(config: MesopicConfig) -> None:
...
def test_coverage_counts_cells_and_not_rows(config: MesopicConfig) -> None:
...
async def test_a_dead_camera_still_shows_its_tiles(config: MesopicConfig, store: Store) -> None:
...
async def test_the_fragment_names_a_stalled_camera(config: MesopicConfig, store: Store) -> None:
...
def _config_without(role: str) -> MesopicConfig:
    parsed: dict[str, Any] = yaml.safe_load(EXAMPLE_CONFIG.read_text(encoding="utf-8"))
    parsed["zones"] = [zone for zone in parsed["zones"] if zone.get("role") != role]
    return MesopicConfig.model_validate(parsed)


def _config_also_collecting(metric: str, *, on_zone: str) -> MesopicConfig:
    parsed: dict[str, Any] = yaml.safe_load(EXAMPLE_CONFIG.read_text(encoding="utf-8"))
...
def test_the_default_cohort_leaves_every_reading_alone(config: MesopicConfig) -> None:
...
def test_customers_are_the_total_less_the_staff_portion(config: MesopicConfig) -> None:
...
def test_the_staff_cohort_shows_the_stored_staff_portion(config: MesopicConfig) -> None:
    rows = [_row(MetricName.OCCUPANCY, 10.0, staff_value=3.0, minutes_ago=0)]
...
def test_a_counting_metric_sums_its_cohort_across_the_window(config: MesopicConfig) -> None:
...
def test_one_unmeasured_bucket_makes_the_whole_total_absent(config: MesopicConfig) -> None:
...
def test_a_mean_over_two_populations_cannot_be_split(config: MesopicConfig) -> None:
...
def test_the_charts_follow_the_cohort_too(config: MesopicConfig) -> None:
