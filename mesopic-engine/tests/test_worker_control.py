...
numbers, never frames": `mesopic.api` may not import a codec, so the worker that already
...
from mesopic.errors import SnapshotUnavailableError, StreamDropped
from mesopic.store.store import Store
from mesopic.supervisor.control import Reconfigure, Retarget, Snapshot, SnapshotReply, Stop
from mesopic.supervisor.snapshot import MAX_EDGE_PX, encode_snapshot
from mesopic.supervisor.supervisor import Supervisor
from mesopic.supervisor.worker import camera_loop
...
async def test_a_refusal_never_names_the_source_url(tmp_path: Path, config: MesopicConfig) -> None:
...
async def test_reload_compiles_before_it_sends(tmp_path: Path, config: MesopicConfig) -> None:
