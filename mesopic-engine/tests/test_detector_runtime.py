...
from mesopic.detector.runtime import (
    DEFAULT_RUNTIME,
    RUNTIME_ENV_VAR,
    artefact_tag,
    available_runtimes,
...
def test_runtime_values_are_stable_wire_strings() -> None:
    ...
    An operator types them into `MESOPIC_DETECTOR_RUNTIME` and P2.1 will accept them in
    `mesopic.yaml`, so renaming one silently breaks a deployment that was working.
    """
...
def test_auto_selection_does_not_pick_an_accelerator(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr("mesopic.detector.runtime.is_available", lambda _runtime: True)
