...
# Assembled, never spelled out. The tests below need a password that *varies* — that is
# the whole point of them — and a credential-shaped RTSP literal is exactly what
# `.gitleaks.toml` scans the repository for, correctly, whether or not the credential in
# it is a real one. Composing the URL keeps the only written-out credential in this file
# the documented placeholder above.
_URL_TEMPLATE = "rtsp://{credential}@{host}:554{path}"


def _url_carrying(password: str, *, user: str = "user", path: str = CAMERA_PATH) -> str:
    return _URL_TEMPLATE.format(credential=f"{user}:{password}", host=CAMERA_HOST, path=path)
...
def test_no_password_survives_redaction(password: str) -> None:
    ...
    assume(password not in f"rtsp://user@{CAMERA_HOST}:554{CAMERA_PATH}")
...
def test_scrub_removes_a_password_quoted_on_its_own() -> None:
    ...
    url = _url_carrying("s3cr3t-lens", path="/stream1")
...
def test_scrub_does_not_shred_a_diagnostic_around_a_very_short_password() -> None:
    ...
    url = _url_carrying("1", path="/stream1")
