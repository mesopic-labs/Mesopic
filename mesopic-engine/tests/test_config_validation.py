"""What `mesopic.yaml` validation must reject, and what it must accept.

Written red-first as MK.3, against a `MesopicConfig` that had no fields and a
...
Fixtures build on the worked example (`examples/mesopic.yaml`) rather than inventing a
...
from mesopic.config.schema import RtspSource
...
def test_camera_url_may_be_an_env_reference(valid_config: dict[str, Any], tmp_path: Path) -> None:
    ...
    del source["url"]
    source["url_env"] = "MESOPIC_FRONT_DOOR_RTSP"
    path = _write(tmp_path, valid_config)
    ...
    assert source.url_env == "MESOPIC_FRONT_DOOR_RTSP"
...
def test_the_api_password_is_referenced_by_env_var_name(
    ...
    valid_config["api"] = {"password_env": "MESOPIC_ADMIN_PASSWORD"}
    path = _write(tmp_path, valid_config)

    config = load_config(path)

    assert config.api.password_env == "MESOPIC_ADMIN_PASSWORD"  # noqa: S105 - an env-var name
