...
from mesopic.config.schema import ExportersConfig, MesopicConfig, MqttExporterConfig
from mesopic.errors import ExportError
from mesopic.exporters.csv_export import CsvExporter
from mesopic.exporters.fanout import build_exporters
from mesopic.exporters.mqtt import MqttExporter
from mesopic.exporters.prometheus import PrometheusExporter
from mesopic.exporters.webhook import SIGNATURE_HEADER, WebhookExporter
from mesopic.types import CameraId, MetricName, MetricRow, MinuteBucket, ScopeId, SiteId
...
def config() -> MesopicConfig:
    return MesopicConfig.model_validate(yaml.safe_load(EXAMPLE_CONFIG.read_text(encoding="utf-8")))
...
def _values(client: FakeMqttClient) -> list[tuple[str, str, bool]]:
    ...
    return [
        ...
        if topic.startswith("mesopic/") and topic != "mesopic/status"
    ]
...
def test_the_mqtt_topic_names_camera_metric_and_scope() -> None:
    client = FakeMqttClient()
    exporter = MqttExporter("broker.invalid", base_topic="mesopic", client=client)
    exporter.start()

    exporter.on_metric(_row())
    exporter.shutdown()

    assert _values(client)[0][0] == "mesopic/front-door/footfall/door-count"


def test_a_camera_wide_metric_gets_a_topic_without_a_scope_segment() -> None:
    """An empty segment (`mesopic/cam/occupancy//`) is not a topic anyone can subscribe to."""
    client = FakeMqttClient()
    exporter = MqttExporter("broker.invalid", client=client)
    exporter.start()
    ...
    assert _values(client)[0][0] == "mesopic/front-door/occupancy"
...
def test_prometheus_exports_engine_health_not_only_business_metrics() -> None:
    ...
    assert "mesopic_live_workers" in rendered
    assert "mesopic_restarts" in rendered
...
def test_a_sensor_is_announced_the_first_time_a_metric_arrives() -> None:
    ...
    assert list(_discovery(client)) == [
        "homeassistant/sensor/mesopic_acme-camden_front-door_footfall_door-count/config"
    ]
...
def test_the_discovery_config_is_retained() -> None:
    """It is how HA rebuilds its entities after a restart with Mesopic still running."""
    client = FakeMqttClient()
    exporter = _ha_exporter(client)
    exporter.start()
...
def test_the_sensors_of_one_camera_are_one_device() -> None:
    ...
    (config,) = _discovery(client).values()
    assert config["device"]["identifiers"] == ["mesopic_acme-camden_front-door"]
...
def test_a_heatmap_is_not_announced_as_a_sensor() -> None:
    ...
    dashboard that lists Mesopic's sensors."""
    client = FakeMqttClient()
    exporter = _ha_exporter(client)
    exporter.start()
...
def test_discovery_can_be_turned_off_without_turning_off_the_values() -> None:
    ...
    assert _discovery(client) == {}
    assert any(topic.startswith("mesopic/") for topic, _p, _r in client.published)
...
def test_the_will_says_offline_on_the_same_topic_the_sensors_watch() -> None:
    ...
    (config,) = _discovery(client).values()
    assert client.will == ("mesopic/status", "offline", True)
    assert config["availability_topic"] == "mesopic/status"


def test_the_built_exporter_knows_the_site_it_is_announcing(config: MesopicConfig) -> None:
    ...
    under a default site id, so two Mesopic boxes on one broker would collide on every
