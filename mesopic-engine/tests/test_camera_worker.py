...
from mesopic.errors import ConfigError, StreamDropped
from mesopic.supervisor.control import Heartbeat
from mesopic.supervisor.pipeline import build_pipeline
...
def test_a_url_env_camera_reads_its_url_from_the_environment(
    ...
    monkeypatch.setenv("MESOPIC_FRONT_DOOR_RTSP", EXAMPLE_RTSP_URL)
    camera = next(c for c in raw_config["cameras"] if c["camera_id"] == "front-door")
    del camera["source"]["url"]
    camera["source"]["url_env"] = "MESOPIC_FRONT_DOOR_RTSP"
    config = MesopicConfig.model_validate(raw_config)
...
def test_a_missing_url_env_var_is_refused_without_naming_its_value(
    ...
    monkeypatch.delenv("MESOPIC_FRONT_DOOR_RTSP", raising=False)
    camera = next(c for c in raw_config["cameras"] if c["camera_id"] == "front-door")
    del camera["source"]["url"]
    camera["source"]["url_env"] = "MESOPIC_FRONT_DOOR_RTSP"
    config = MesopicConfig.model_validate(raw_config)

    with pytest.raises(ConfigError, match="MESOPIC_FRONT_DOOR_RTSP"):
...
def test_an_unsupported_source_is_refused_without_leaking_the_url(
    ...
    *without* printing a URL, and that outlives whichever kind is currently unsupported.
    ...
    raw_config["cameras"][1]["source"] = {
        "kind": "onvif",
        "host": "192.0.2.10",
        "username_env": "MESOPIC_TILL_USER",
        "password_env": "MESOPIC_TILL_PASSWORD",
    }
    config = MesopicConfig.model_validate(raw_config)
