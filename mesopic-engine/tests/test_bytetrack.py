...
from mesopic.tracker.cost import (
    CostFunction,
    ceiling_for,
...
from mesopic.tracker.kalman import _MAX_WALK_SPEED_MS, _PERSON_HEIGHT_M
from mesopic.types import CameraId, DecodedFrame, Detection, FrameTs, TrackId
...
def test_plain_iou_swaps_identities_where_the_default_cost_does_not() -> None:
    ...
    gate widens enough to rescue a cold-start match at Mesopic's fps range, the same
...
def test_a_brisk_walker_is_admitted_at_every_supported_frame_rate(fps: int) -> None:
    ...
    at every fps Mesopic supports -- not just whichever one someone currently believes
    is tightest.
...
def test_the_additive_gate_has_a_crossover_fps_below_which_it_refuses_nothing() -> None:
    ...
    ~= 1.067s`, i.e. **~0.9375 fps** -- just BELOW Mesopic's 1 fps product floor. At
