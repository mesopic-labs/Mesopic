...
from mesopic.supervisor.control import Heartbeat
from mesopic.supervisor.handle import (
    STALL_AFTER_S,
    WorkerEntry,
    WorkerHandle,
    WorkerReport,
...
from mesopic.supervisor.worker import HEARTBEAT_INTERVAL_S, HeartbeatEmitter
...
def _handle(config: MesopicConfig, entry: WorkerEntry) -> WorkerHandle:
...
def test_a_handle_reports_what_the_worker_actually_sent(config: MesopicConfig) -> None:
...
def test_a_worker_that_has_sent_nothing_reports_no_frame_time(config: MesopicConfig) -> None:
...
def test_draining_keeps_the_newest_of_several_queued_heartbeats(config: MesopicConfig) -> None:
...
def test_a_restart_clears_the_dead_workers_heartbeat(config: MesopicConfig) -> None:
